...
// ── Draw watermark centered on a single PDF page ──────────────────────────────
async function drawWatermark(doc, dataUrl) {
  if (!dataUrl) return;
  await loadImg(dataUrl);

  // Calculate centered watermark size (40% of page width)
  const wmW = PAGE_W * 0.4;
  const wmH = PAGE_H * 0.4;
  const wmX = (PAGE_W - wmW) / 2;
  const wmY = (PAGE_H - wmH) / 2;

  // Use jsPDF graphics state for opacity
  const gState = new doc.GState({ opacity: 0.06, "fill-opacity": 0.06 });
  doc.saveGraphicsState();
  doc.setGState(gState);
  doc.addImage(dataUrl, imgFmt(dataUrl), wmX, wmY, wmW, wmH);
...
async function drawTemplateBg(doc, tmpl) {
  ...
  // ── Draw fullBg as centered watermark on this page ──
  if (id === "rich" && slotImages?.fullBg?.dataUrl) {
    await drawWatermark(doc, slotImages.fullBg.dataUrl);
  }

  if (id === "rich" && slotImages) {
    if (slotImages.headerBanner?.dataUrl) {
      await loadImg(slotImages.headerBanner.dataUrl);
      doc.addImage(
        slotImages.headerBanner.dataUrl,
        imgFmt(slotImages.headerBanner.dataUrl),
        0,
        0,
        PAGE_W,
        20,
    ...
    if (slotImages.rightSidebar?.dataUrl) {
      await loadImg(slotImages.rightSidebar.dataUrl);
      doc.addImage(
        slotImages.rightSidebar.dataUrl,
        imgFmt(slotImages.rightSidebar.dataUrl),
        PAGE_W - 15,
        0,
        15,
        PAGE_H,
    ...
    if (slotImages.topRight?.dataUrl) {
      await loadImg(slotImages.topRight.dataUrl);
      doc.addImage(
        slotImages.topRight.dataUrl,
        imgFmt(slotImages.topRight.dataUrl),
        PAGE_W - MR - 30,
        3,
        30,
        25,
    ...
    if (slotImages.bottomLeft?.dataUrl) {
      await loadImg(slotImages.bottomLeft.dataUrl);
      doc.addImage(
        slotImages.bottomLeft.dataUrl,
        imgFmt(slotImages.bottomLeft.dataUrl),
        ML,
        PAGE_H - 28,
        30,
        25,
...
function drawTemplateBorder(doc, tmpl) {
  ...
  switch (tmpl.borderStyle) {
    ...
    case "double":
      ...
      doc.rect(M + 3, M + 3, PAGE_W - (M + 3) * 2, PAGE_H - (M + 3) * 2);
      break;
    case "corner": {
      doc.setDrawColor(r, g, b);
      const arm = 16;
      doc.setLineWidth(1.2);
      doc.line(M, M + arm, M, M);
      doc.line(M, M, M + arm, M);
      doc.line(PAGE_W - M - arm, M, PAGE_W - M, M);
      doc.line(PAGE_W - M, M, PAGE_W - M, M + arm);
      doc.line(PAGE_W - M, PAGE_H - M - arm, PAGE_W - M, PAGE_H - M);
      doc.line(PAGE_W - M, PAGE_H - M, PAGE_W - M - arm, PAGE_H - M);
      doc.line(M + arm, PAGE_H - M, M, PAGE_H - M);
      doc.line(M, PAGE_H - M, M, PAGE_H - M - arm);
      break;
    }
    case "accent":
      doc.setFillColor(r, g, b);
      doc.setDrawColor(r, g, b);
      doc.rect(M, M, 3.5, PAGE_H - M * 2, "F");
      doc.setLineWidth(0.4);
      doc.line(M, M, PAGE_W - M, M);
...
function parseLines(text) {
  return text.split("\n").map((raw) => {
    ...
    if (isColonSubheading(t))
      return { kind: "sub", text: sanitize(t.slice(0, -1)) };
...
export async function generateProposalPdf(
  ...
  function findTocInsertAt() {
    const fi = tocEntries.findIndex(
      (t) => t.indent === 0 && /future\s*scope/i.test(t.title),
    );
    if (fi >= 0) return fi;
    const maj = tocEntries
      .map((t, i) => (t.indent === 0 ? i : -1))
      .filter((i) => i >= 0);
    return maj.length >= 2
      ? maj[maj.length - 2]
      : (maj[maj.length - 1] ?? tocEntries.length);
  }
  ...
  async function pageBreak(n = 10) {
    if (y + n > PAGE_BOTTOM) {
      doc.addPage();
      await drawTemplateBg(doc, tmpl); // ← watermark drawn on every new page
      drawHeader(doc);
      y = 26;
    }
  }
  ...
  for (let idx = 0; idx < parsed.length; idx++) {
    ...
    if (idx === screenshotH1Index) {
      await pageBreak(14);
      y += 5;
      sectionPageMap["DEMO SCREENSHOTS"] = {
        pageNumber: doc.getCurrentPageInfo().pageNumber,
        yMm: y,
      };
      doc.setFontSize(12);
  ...
  for (const pNum of pagesNeedingBg) {
    doc.setPage(pNum);
    await drawTemplateBg(doc, tmpl); // ← watermark drawn here too
    drawHeader(doc);
  }
  ...
  for (let p = 2; p <= total; p++) {
    ...
    if (clientName)
      doc.text(sanitize(`Prepared for: ${clientName}`), ML + 4, footerY);
    if (p >= 3)
      doc.text(String(p - 2), PAGE_W / 2, footerY, { align: "center" });
    if (preparedBy)
      doc.text(sanitize(preparedBy), PAGE_W - MR - 4, footerY, {
        align: "right",
      });
  }
