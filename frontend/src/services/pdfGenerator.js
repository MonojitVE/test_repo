...
// ── drawImage uses a state object so pageBreakFn can update y correctly ───────
async function drawImage(doc, dataUrl, state, pageBreakFn) {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      // pageBreakFn may update state.y — call it before reading state.y for addImage
      pageBreakFn(imgH + 8);
...
export async function generateProposalPdf(
  ...
  // Find Future Scope parsed index by title match (robust vs positional guessing)

  const futureScopeIdx = parsed.findIndex(
    (e) => e.kind === "h1" && /future\s*scope/i.test(e.title),
  );

  // screenshotH1Index: the parsed[] index before which screenshots are injected
  ...
  // demoLinkH1Index: same anchor — used when no screenshots to shift body nums
  ...
  // ── Helper: find the TOC insertion point (before Future Scope) ────────────
  ...
  // ── Inject Screenshots into TOC before Future Scope ───────────────────────
  ...
  if (screenshots.length > 0 && tocEntries.length > 0) {
    ...
    // Shift all major section numbers from insertAt onwards up by 1
  ...
  // ── Inject Demo Link into TOC right after Demo Screenshots ────────────────

  // Works whether screenshots exist or not — always inserts before Future Scope

  if (demoLink.trim()) {
    const ssIdx = tocEntries.findIndex((t) => t.title === "DEMO SCREENSHOTS");

    // If screenshots exist, place demo link right after them.

    // If not, find Future Scope position fresh.

    const demoInsertAt = ssIdx >= 0 ? ssIdx + 1 : findTocInsertAt();

    // The entry at demoInsertAt is Future Scope. It has already been shifted

    // once if screenshots exist. Demo Link takes that number, Future Scope shifts again.

    const demoLinkSectionNum = parseInt(tocEntries[demoInsertAt]?.num || "0");

    // Shift all major section numbers from demoInsertAt onwards up by 1
  ...
  // Use a state object so drawImage's pageBreakFn can update y correctly
  ...
  // How many sections were inserted before Future Scope (0, 1, or 2)

  const insertedBefore =
    (screenshots.length > 0 && screenshotH1Index >= 0 ? 1 : 0) +
    (demoLink.trim() && demoLinkH1Index >= 0 ? 1 : 0);

  // The parsed[] index at which body section numbering must start shifting

  const bodyShiftAnchorIdx =
    screenshotH1Index >= 0 ? screenshotH1Index : demoLinkH1Index;

  // Reusable function to render the Demo Link content block
  ...
  for (let idx = 0; idx < parsed.length; idx++) {
    ...
    // ── Inject screenshots (and demo link) before Future Scope h1 ────────
    ...
    if (idx === screenshotH1Index) {
      ...
      // Demo Link always immediately follows screenshots
    ...
    // ── Inject Demo Link before Future Scope when there are NO screenshots ──
    ...
    switch (e.kind) {
      ...
      case "h1": {
        ...
        // Shift body h1 numbers by however many sections were inserted before Future Scope
