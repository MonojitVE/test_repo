...
async function drawImage(doc, dataUrl, state, pageBreakFn) {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      const maxW = CW, maxH = 120;
      let imgW = maxW, imgH = imgW / aspectRatio;
...
export async function generateProposalPdf(
  ...
  // Screenshots go before second-to-last h1 (before "FUTURE SCOPE")
  // so order: ..., Screenshots, Future Scope, Time & Budget — matching preview
  const screenshotH1Index = screenshots.length > 0
    ? (h1Indices.length >= 2 ? h1Indices[h1Indices.length - 2] : h1Indices[h1Indices.length - 1] ?? -1)
    : -1;
  ...
  // ── Inject Screenshots into TOC before second-to-last major section ───────
  if (screenshots.length > 0 && tocEntries.length > 0) {
    const majorIndices = tocEntries
      .map((t, i) => (t.indent === 0 ? i : -1))
      .filter(i => i >= 0);

    const insertAt = majorIndices.length >= 2
      ? majorIndices[majorIndices.length - 2]
      : majorIndices[majorIndices.length - 1];
  ...
  // ── Inject Demo Link into TOC at the very end ─────────────────────────────
  if (demoLink.trim()) {
    simBreak(14);
    tocEntries.push({
      num: "",
      title: sanitize(demoLabel.trim() || "Demo Link"),
      page: simPage,
      indent: 0,
    });
  }
  ...
  const yState = { get y() { return y; } };

  function pageBreak(n = 10) {
    if (y + n > PAGE_BOTTOM) { doc.addPage(); drawHeader(doc); y = 26; }
  }
  function rule() {
    pageBreak(5);
    sd(doc, RULE);
  ...
  for (let idx = 0; idx < parsed.length; idx++) {
    ...
    // ── Inject screenshots before the correct h1 ──────────────────────────
    if (idx === screenshotH1Index) {
      pageBreak(14); y += 5;
      ...
      for (let si = 0; si < screenshots.length; si++) {
        ...
        const imgH = await drawImage(doc, s.dataUrl, yState, (needed) => {
          if (y + needed > PAGE_BOTTOM) { doc.addPage(); drawHeader(doc); y = 26; }
        });
    ...
    switch (e.kind) {
      ...
      case "h1": {
        ...
        // Shift section numbers for all h1s from screenshotH1Index onwards
        let displayNum = e.num;
        if (screenshots.length > 0 && screenshotH1Index >= 0) {
          const thisPos = h1Indices.indexOf(idx);
          const insertPos = h1Indices.indexOf(screenshotH1Index);
          if (thisPos >= insertPos) {
            displayNum = String(parseInt(e.num) + 1);
          }
        }
  ...
  // ── Demo Link section — always last ───────────────────────────────────────
  if (demoLink.trim()) {
    pageBreak(14);
    y += 5;
    doc.setFontSize(12); doc.setFont("helvetica", "bold"); sc(doc, BLUE);
    doc.text("DEMO LINK", ML, y);
    y += 2;
    rule();
